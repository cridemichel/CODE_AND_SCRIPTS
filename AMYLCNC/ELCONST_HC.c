  ...
  for (i=0; i < 2; i++)
    {
      ...
      amyloids[i].Lbox= AMY_LBOX_ALONG_AXIS;/* altezza lungo l'asse dell'elica del box in nm, l'unico vincolo 
					      è che sia abbastanza più piccola
...
#else
      amyloids[i].boxsax[0] = 1.01*sqrt(Sqr(amyloids[i].nD*amyloids[i].Dproto)+Sqr(amyloids[i].ribthick))/2.0;
