...
int main(int argc, char **argv)
{
...
#else
     printf("create_one_attached <conf_file_name> <nxmax> <nymax> <nzmax> <Lx> <Ly> <Lz> <DensSuperfAntigens> <numspheres> <QFab-diam> <QFab-len> <QFab-diam-permpatch> <QFab-diam-revpatch> <QFab-dist-revpatch> <sphere-diam> <sphere-permpatch-diam> <DiametroAntigene> <bigAntigenSurfDiam> <maxnanobodies>\n"); 
#endif
     exit(-1);
   }
  f = fopen(argv[1], "w+");
  ...
  fprintf(f,"%f %f %f\n", DiamSph/2.0, DiamSph/2.0, DiamSph/2.0); /* each dsDNA of 48 bp which is roughly equal to 48 / 3 nm = 16 nm (D=2 nm in our case) */ 
  fprintf(f,"1 1 1\n");
  fprintf(f, "%f %f %f %f 1 0\n", massBranch, IBranch, IBranch, IBranch);
  fprintf(f,"%d 0\n", numarms);
  for (k1=0; k1 < numarms; k1++)
    {
  ...
  fprintf(f,"0 1 3 0 1 1000000 1000000 1\n");
