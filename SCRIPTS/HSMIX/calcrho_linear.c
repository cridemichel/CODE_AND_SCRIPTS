...
int main(int argc, char **argv)
{
 ...
 if ((A0 > B0 && A0 > C0) || (A0 < B0 && A0 < C0))
  ...
  if (NPA < NP)
    {
      dir=opendir("./RHOTMPLINA");
      if (errno==ENOENT)
	system("mkdir RHOTMPLINA/");
      else
	closedir(dir);
      dir=opendir("./RHOTMPLINB");
      if (errno==ENOENT)
	system("mkdir RHOTMPLINB/");
      else
	closedir(dir);
      f=fopen("./RHOTMPLINA/NN.dat","w");
      fprintf(f, "%d\n", NN);
      fclose(f);
      f=fopen("./RHOTMPLINB/NN.dat","w");
      fprintf(f, "%d\n", NN);
      fclose(f);
    }
  else
    {
      dir=opendir("./RHOTMPLIN");
      if (errno==ENOENT)
	system("mkdir RHOTMPLIN/");
      else
	closedir(dir);
      f=fopen("./RHOTMPLIN/NN.dat","w");
      fprintf(f, "%d\n", NN);
      fclose(f);
  ...
  for (nr1 = 0; nr1 < nfiles; nr1++)
      ...
      for (qmod = qmin; qmod <= qmax; qmod++)
	{
	  ...
	  if (NPA < NP)
	    {
	      sprintf(fname2,"RHOTMPLINA/ro.00.k=%03d", qmod);
	      fA=fopen(fname2,mode);
	      sprintf(fname2,"RHOTMPLINB/ro.00.k=%03d", qmod);
	      fB=fopen(fname2,mode);
      	    }
	  else 
	    {
	      sprintf(fname2,"RHOTMPLIN/ro.00.k=%03d", qmod);
	      f=fopen(fname2,mode);
	    }
	  if (NPA < NP)
	    {
