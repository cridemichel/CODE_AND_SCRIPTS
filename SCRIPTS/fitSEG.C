...
// par[4] è \omega_q = q*v = q*\sqrt{k_BT/m}
Double_t fitFunctionStrechedGauss(Double_t *x, Double_t* par)
{
  return par[0]*(par[3]*exp(-pow(x[0]/par[1],par[2]))+(1.0-par[3])*exp(-x[0]*x[0]*par[4]/2.0));
}
...
void fitSEG(char *fileName=NULL, Float_t omegaq= - 1.0, Int_t type=0, Float_t beg=0.0, Float_t end=0.0, Int_t llogx=0, Int_t llogy=0,
	   Int_t xpnts=-1, Int_t ypnts=-1)
{
  ...
  if (fileName==NULL)
    {
      ...
      printf("Usage: root.exe 'fitSEG.C(<filename>,<omegaq>,<type>,<beg>,<end>,<logx>,<logy>,<xpnts>,<ypnts>)\n");
  ...
  fitFcn = new TF1("fitFcn",fitFunctionStrechedGauss, minXF, maxXF, 5);
 // create a TF1 with the range from 0 to 3 and 6 parameters
  ...
  if (type==2||type==3)
    {
      fitFcn->GetParameters(par);
      fitFcn->SetParameters(par[0],par[1],1.0,1.0,0.0);
      if (type==3)
	{
	  fitFcn->FixParameter(3,1.0);
	  fitFcn->FixParameter(4,0.0);
