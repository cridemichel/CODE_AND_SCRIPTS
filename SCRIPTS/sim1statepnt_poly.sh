...
# $5 = label for different run (farm)
#      $4 passi di equilibratura ( 0 = default ) 
if [ "$1" = "" ]
then
echo "Syntax: sim1statepnt <Volume_Fraction> <production_cycles> [elongation] [equilibration_steps] [run label]"
exit 0
fi
if [ "$2" = "" ]
then
...
../../set_params.sh rand_$PARFILE stepnum 100 polydispX $PDX polydispY $PDY polydispZ $PDZ targetPhi 0.0 storerate 0.0 intervalSum 0.2 scalevel 1 rcut $RCUT rNebrShell $RNNL endfile ${SIMRA}.cor parnum $PARNUM A0 $A0 B0 $B0 C0 $C0
ln -sf $ELLEXE $SIMRA
$SIMRA -f ./rand_${PARFILE} > screen_$SIMRA 
#exit 
#>>> EQUILIBRATE STARTING DENSITY (I.E. RANDOMIZE)
../../set_params.sh rand_$PARFILE stepnum 1000 polydispX 0.0 polydispY 0.0 polydispZ 0.0 targetPhi 0.0 storerate 0.0 intervalSum 2.0 scalevel 0 inifile ${SIMRA}.cor endfile ${SIMRA}.cor 
ln -sf $ELLEXE $SIMRA 
$SIMRA -f ./rand_${PARFILE} >> screen_$SIMRA 
#CRESCITA
