...
int main(int argc, char** argv)
{
 ...
 fprintf(f, "@@@\n");
 /* positions of receptors: atof(argv[1]) is the geometrical distance between receptors*/
 rr = (atof(argv[1])+sigRecept)/sqrt(3.0);
 fprintf(f, "%.15G %.15G %.15G 1 0 0 0 1 0 0 0 1 0\n",                0.0,      rr, -L*0.5);
 fprintf(f, "%.15G %.15G %.15G 1 0 0 0 1 0 0 0 1 0\n", rr*(sqrt(3.0)/2.0), -rr/2.0, -L*0.5);
 fprintf(f, "%.15G %.15G %.15G 1 0 0 0 1 0 0 0 1 0\n",-rr*(sqrt(3.0)/2.0), -rr/2.0, -L*0.5);
 /* positions of proteins */
 for (i=0; i < Nprot; i++)
   {
     fprintf(f, "%.15G %.15G %.15G 1 0 0 0 1 0 0 0 1 1\n", (L-2.0)*(ranf()-0.5)+0.5, (L-2.0)*(ranf()-0.5)+0.5,
	     (L-2.0)*(ranf()-0.5)+0.5); 
   }
 /* velocities */
