...
void init_parameters(void)
{
  ...
  massAmino = 1.0;
  /* check these values! */
  //sigCBGM  = 3.07;/* hard core interaction for CB: already implemented, see sigBB interaction! */
  sigCB2GM = 7.5; /* attraction range for CB within go-model */  
  /* ------------------- */
#ifdef PEPTIDE_PLATE
  massPlate = 1.0;
...
void readGoMatrix(char *fn)
{
  ...
  if (!(f = fopen(fn,"r")))
    {
      printf("problem opening go matrix file %s\n", fn);
      exit(-1);	
    }
  i=0;
  while (!feof(f))
  ...
  printf("2-reading go matrix!\n");
  ngointer = i;
  goArr = malloc(sizeof(goPairType)*ngointer);
  rewind(f);
