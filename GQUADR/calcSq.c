...
int readCnf = 0, physunit=0, k1;
#define KMODMAX 599
#define NKSHELL 150
double qx[KMODMAX][NKSHELL], qy[KMODMAX][NKSHELL], qz[KMODMAX][NKSHELL];
double qavg[KMODMAX];
int qmin=0, qmax=KMODMAX-1, eventDriven = 0;
double qminpu=-1.0, qmaxpu=-1.0, sfAvg;
int ntripl[]=
#include "./ntripl.dat"
int mesh[][NKSHELL][3]= 
#include "./kmesh.dat"
double twopi;
double Sq[KMODMAX], sumRho, reRho, imRho, rCMk, scalFact[3], invNm, invL[3], L[3], invNmAA, invNmBB, invNmAB;
...
int main(int argc, char** argv)
{
  ...
  while (!feof(f2))
      ...
      if (first)
	{
	  if (physunit || qminpu != -1.0 || qmaxpu != -1.0)
	      ...
	      for (qmod = 0; qmod < KMODMAX; qmod++)
		{
		  ...
		  for (mp = 0; mp < ntripl[qmod]; mp++) 
		    {
		      qavg[qmod] += sqrt(Sqr(scalFact[0]*((double)mesh[qmod][mp][0]))+
					 Sqr(scalFact[1]*((double)mesh[qmod][mp][1]))+
					 Sqr(scalFact[2]*((double)mesh[qmod][mp][2])));
		    }
		  qavg[qmod] *= 1.0/((double)ntripl[qmod]);
		  //printf("qavg[%d]:%.15G - %.15G\n", qmod, qavg[qmod], scalFact*(1.25+0.5*qmod));
		}
              sfAvg = (scalFact[0]+scalFact[1]+scalFact[2])/3.0;
	      if (qminpu != -1.0 && qminpu == qmaxpu)
		{
		  qmin = rint((qminpu-1.0) / (sfAvg/2.0));
		  qmax = qmin;
		}
	      else 
		{
		  if (qminpu != -1.0 && qminpu >= 0.0)
		    qmin = ceil( qminpu / (sfAvg/2.0) - 2.0);
		  if (qmaxpu != -1.0 && qmaxpu > 0.0)
		    qmax = floor( qmaxpu / (sfAvg/2.0) - 2.0);
		}
	      if (qmin < 0)
		qmin = 0;
	      if (qmin >= KMODMAX)
	  ...
	  printf("qmin: %d qmax: %d qminpu: %.15G qmaxpu: %.15G\n", qmin, qmax, qminpu, qmaxpu);
	      ...
	      for(mp = 0; mp < ntripl[n]; mp++)
		{
		  ...
		  for(i=0; i < N; i++)
			  ...
			  mesh[n][mp][2] == 0)
			{
		      ...
		      rCMk = kbeg +  
			(r[0][i] * mesh[n][mp][0] * scalFact[0] + r[1][i] * mesh[n][mp][1] * scalFact[1] + 
			 r[2][i] * mesh[n][mp][2] * scalFact[2]);
