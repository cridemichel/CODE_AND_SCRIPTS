...
char **fname; 
double L, time, *ti, *rotMSD, *MSD, *rotMSDA, *MSDA, *rotMSDB, *MSDB, *cc, **rotMSDcls, **MSDcls, **cc_cls;
double **DR, **DR0;
...
int main(int argc, char **argv)
{
  ...
  cc_cls = malloc(sizeof(double*)*NP);
  MSDcls = malloc(sizeof(double*)*NP);
  rotMSDcls = malloc(sizeof(double*)*NP);
  for (kk=0; kk < NP; kk++)
    {
  ...
  for (kk=0; kk < NP; kk++)
    {
      sprintf(dummy, "MSD%d.dat", kk);
      f = fopen(dummy, "w+");
      for (ii=1; ii < points; ii++)
	{
	  //printf("cc[%d]=%f ti=%f\n", ii, cc[ii], ti[ii]);
	  if (cc_cls[kk][ii] > 0 && ti[ii] > -1.0)
	    {
	      fprintf(f, "%.15G %.15G %f\n", ti[ii]-ti[0], MSDcls[kk][ii]/cc_cls[kk][ii], cc_cls[kk][ii]);
	      if (foundrot)
		fprintf(f2, "%.15G %.15G %f\n", ti[ii]-ti[0], rotMSDcls[kk][ii]/cc_cls[kk][ii], cc_cls[kk][ii]);
	    }
	}
      fclose(f);
    }
