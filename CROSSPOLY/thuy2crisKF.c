...
void versors_to_R(double *r1, double *r2, double R[3][3])
{
  int k;
...
{
  FILE* f, *f2;
  int i, kk, nummol, junki1, junki2, junki3, passi, polylen;
  double sf, radius, junkd1, junkd2, junkd3, trad, patchRad, patchPos, tlen, prad, patchrange, permrange;
...
*/
  fprintf(f2,"ninters: 4\nnintersIJ: 0\nntypes: 1\nsaveBonds: 0\n@@@\n");
  fprintf(f2,"%d\n", nummol);	
  fprintf(f2,"%G %G %G\n", prad, prad, prad);	
  fprintf(f2,"2 2 2\n");
  fprintf(f2,"1 1 1 1 2 0\n");
  fprintf(f2,"3 0\n");
  fprintf(f2, "1.0 0 0 %f\n", permrange*2.0);
  fprintf(f2, "-1.0 0 0 %f\n", permrange*2.0);
  fprintf(f2, "0 1.0 0 0.5\n");
  ...
  fprintf(f2, "0 2 0 2 1 0 0 100000\n");
  fprintf(f2,"@@@\n");
  for (i=0; i < nummol; i++)
    {
      fscanf(f, "%lf %lf %lf\n", &r1[0], &r1[1], &r1[2]);
