...
int main(int argc, char **argv)
{
  ...
  double phi, targetphi=0.50, xtrafact, permdispl;
  int k1, k2, numpoly, parnum=1000, i, j, polylen=1, a, b;
  int nx, ny, nz, nxmax, nymax, nzmax, idx;
  del=0.5;
  /* permanent spots diameter */
  permdiam=0.3; /* 20 T * 0.63 nm dove 0.63 nm è la lunghezza per base stimata per ssDNA in BiophysJ 86, 2630 (2004) */  
  permdispl=0.18;
  //permdiam=0.5;
  Len=0.34; /* 48 bp equal roughly to 16 nm */
  if (argc == 1)
   {

     printf("create_GDNA_conf_nematic_unfolded <conf_file_name> <nxmax> <nymax> <nzmax> <phi> <diam>\n"); 
     exit(-1);
   }
  f = fopen(argv[1], "w+");
  if (f==NULL)
  ...
  fprintf(f,"2 0\n");
  fprintf(f,"%f 0 0 %f\n", permdispl, permdiam);/* 0: along x axis (permanent) 0.05 means lp=20 */
  fprintf(f,"%f 0 0 %f\n", -permdispl, permdiam);
