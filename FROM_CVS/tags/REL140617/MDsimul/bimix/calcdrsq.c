...
int main(int argc, char **argv)
{
  ...
  for (t1= 1; t1 < njob; t1++)
      ...
      for (t2 = t1; t2 < njob; t2++)
	{
	  ...
	  steps1 = Oparams.curStep;
	  strcpy(filewalter, percorso);
	  strcat(filewalter, filename[t2]);
	  readRestart(filewalter);
	  for (i=0; i < Oparams.parnum[0]; i++)
	    {
	      r2x[i] = rx[0][i];
	  ...
	  if (steps2 > steps1)
	    {
	      ...
	      for (i=0;  i < Oparams.parnum[0]; i++)
	       {
		 fpn += Sqr(r2x[i]-r1x[i])+Sqr(r2y[i]-r1y[i])+Sqr(r2z[i]-r1z[i]);
	       }
	      fpn /= ((double)Oparams.parnum[0]);
	      aggiungi = 1;
	      nn = maxt;
	      for (i = 0; i < maxt; i++)
		{
