...
extern int iniBakFormat, iniCorFormat;
/* ================================= */
...
void usrInitBef(void)
{
  ...
  for (i = 0; i < MAXPAR; i++)
    {
      OprogStatus.sumVx[i] = 0.0;
      OprogStatus.sumVy[i] = 0.0;
      OprogStatus.sumVz[i] = 0.0;
    }
  OprogStatus.RW = 0;
  OprogStatus.srescale = 0;
  OprogStatus.linearDiff = 0; 
  OprogStatus.DtOptL = 0;
  OprogStatus.optL = 0;
  OprogStatus.optFact = 0.1;
  OprogStatus.TARGET_RATE = 0.3;
  OprogStatus.RATE_TOL = 0.1;
...
void usrInitAft(void)
{
  ...
  if (iniBakFormat == 1 || iniCorFormat == 1)
    {
      printf("[%d]qui!!!!!!!!!!!!\n", my_rank);
      /* file ascci metto solo posizioni e velocità dunque devo calcolare le 
	 forze e le velocità ai tempi vecchi */
      if (OprogStatus.noLinkedList)
	{
...
/* Notare nei file ascii salvo solo posizioni e velocità */
