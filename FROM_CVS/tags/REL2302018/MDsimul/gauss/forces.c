...
#else
      rxc = rx[i] - L * RINT_(invL * rx[i]);
      ryc = ry[i] - L * RINT_(invL * ry[i]);
      rzc = rz[i] - L * RINT_(invL * rz[i]);
#endif	  
...
void BuildNebrListNoLinked(int Nm, COORD_TYPE rCut, COORD_TYPE sigma) 
{
  ...
  loop(i, 1, Nm - 1)
	...
	/*   i > j because of 3rd law */
	{
	  ...
	  ryij = ryi - ry[j];
	  rzij = rzi - rz[j];
#if defined(MD_USE_RINT) 
	  rxij = rxij - L * rint(rxij * invL);      /* minimum image */
	  ryij = ryij - L * rint(ryij * invL);
	  rzij = rzij - L * rint(rzij * invL);
#else
	  rxij = rxij - L * RINT_(rxij * invL);      /* minimum image */
	  ryij = ryij - L * RINT_(ryij * invL);
	  rzij = rzij - L * RINT_(rzij * invL);
#endif 
...
#else
	      rxij = rxij - L * RINT_(invL * rxij);      /* minimum image */
	      ryij = ryij - L * RINT_(invL * ryij);
	      rzij = rzij - L * RINT_(invL * rzij);
#endif
  ...
  for (n = 0; n < nebrTabLen; n++)
    {
      ...
      Fzi = Fz[i];
#endif
      rxij = rx[i] - rx[j]; /* distance between two atomes */
      ryij = ry[i] - ry[j];
      rzij = rz[i] - rz[j];
#ifdef MD_USE_RINT
      rxij = rxij - L * rint(invL * rxij);      /* minimum image */
      ...
      if ( rijSq < rcutSq )/* 'rcut' is the cutoff for V */
	{
	  ...
	  /* Calculate all temrs of molecular
	     pressure tensor */
	  Fx[i]   = Fx[i] + fxij;     /* total force on an atom (a,i)*/
	  Fy[i]   = Fy[i] + fyij;
	  Fz[i]   = Fz[i] + fzij;
