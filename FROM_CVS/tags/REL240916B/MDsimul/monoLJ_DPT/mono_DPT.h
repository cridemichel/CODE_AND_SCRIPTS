...
#define NUM_MISURE 12         /* maximum number of
...
#undef EXT_PT_DLST
#define EXT_PT_DLST s, s1, s2, s1o1, s1o2
...
struct progStatus
{
  ...
  int bakStepsAscii;    /* steps between two savings of restore files on HaD*/
  ...
  /* ACCUMULATORI DA SCAMBIARE <<< ============================================== ######
     NOTA FONDAMENTALE:
     Tutti questi accumulatori vanno scambiati se due sistemi si scambiano
     la temperatura!!! */
  ...
  /* queste grandezze(g(r) S(k) e MB) eventualmente ce le calcolaiamo off-line */
  ...
  C_T sumVy[MAXPAR];
  C_T sumVz[MAXPAR];
  /* FINE ACCUMULATORI DA SCAMBIARE <<<=========================================  ###### 
   */
  COORD_TYPE Q;
  COORD_TYPE W;
  ...
  */
  ...
  int NUM_PEij; 
  /* Numeri di passi dopo i quali deve ottimizzare i lambda */
  int DtOptL; /* numero di tentativi di scambio dopo i quali ottimizzare le 
		 temperature (tipc. 50-100) */
  int optL; /* 1 = ottimizzatore attivo 0 = disattivo 
	       una volta attivato diventa 0 quando si raggiunge il rate obiettivo */
  double optFact; /* fattore moltiplicativo con il quale si ricava la variazione
		     dei lambda (percentuale cioè deve essere un valore 
		     tra 0 e 1 ) */
  double RATE_TOL; 
  double TARGET_RATE;
  char nRun[32];
...
struct pascii opro_ascii[] =
{
...
#if 0
  //tanto per ora la viscosità non ci interessa 
...
#endif
  /*le grandezze commentate possono essere ricavate con un'analisi off-line
    cosi' si risparmia spazio su HD */
#if 0  
  {"hist",         OS(hist),                  MAXBIN,   1,   "%d"},
  {"sumS",         OS(sumS),                    NUMK,   1, "%.6f"},
  {"histMB",       OS(histMB),                  NUMV,   1,   "%d"},
#endif 
  {"sumTemp",      &OS(sumTemp),                    1,   1, "%.6G"},
  {"sumPress",     &OS(sumPress),                   1,   1, "%.6G"},
  {"sumMedia",     &OS(sumMedia),                   1,   1, "%.6G"},
#if 0  
  {"sumVx",        OS(sumVx),                    MAXPAR, 1,  "%.10G"},
  {"sumVy",        OS(sumVy),                    MAXPAR,  1, "%.10G"},
  {"sumVz",        OS(sumVz),                    MAXPAR, 1, "%.10G"},
#endif  
  {"Q",            &OS(Q),                          1,   1, "%.6g"},
  {"W",            &OS(W),                          1,   1, "%.6G"},
  ...
  {"DtOptL",       &OS(DtOptL),                     1,   1,   "%d"},
  {"optL",         &OS(optL),                       1,   1,   "%d"},
  {"optFact",      &OS(optFact),                    1,   1, "%.8G"},
  {"RATE_TOL",     &OS(RATE_TOL),                   1,   1, "%.8G"},
  {"nRun",         OS(nRun),                       1,    32,  "%s"},
  {"", NULL, 0, 0, ""}
};
#else
...
struct pascii opar_ascii[]=
{
  ...
  {"M",                 &OP(M),                           1,    1,  "%d"},
  {"tol",               &OP(tol),                         1,  1, "%.15G"},
  {"lambda0",           OP(lambda0),                  MAX_M,  1, "%.15G"},
  {"lambdat",           OP(lambdat),                  MAX_M,  1,    "%d"},
  {"PTM",               &OP(PTM),                         1,  1,   "%d"},
  {"", NULL, 0, 0, ""}
};
#else
...
struct singlePar OsinglePar[] = { 
  ...
  */
  ...
  {"DtOptL",     &OprogStatus.DtOptL,        INT},
  {"optFact",    &OprogStatus.optFact,      CT},
  {"optL",       &OprogStatus.optL,         INT},
  {"rateTol",    &OprogStatus.RATE_TOL,     CT},
  {"targetRate", &OprogStatus.TARGET_RATE,  CT},
...
#ifdef MAIN
COORD_TYPE E, Dtrans, temp, S[NUMK], dummy, eta, gr[MAXBIN], 
  invs, press, diffMedia, probSc, 
  press_m, press_at, PE[MAX_M*PE_POINTS], PEijM[2*MAX_M*PE_POINTS];
COORD_TYPE Ptens[3], DQtens[3], 
  sqrtdr2, Aa, DrSqTot, temp_transl, smed;
int MB[NUMV], PEint[MAX_M][PE_POINTS];
#else 
extern COORD_TYPE E, Dtrans, temp, S[NUMK], 
  dummy, eta, probSc, 
  gr[MAXBIN], invs, diffMedia,
  press, press_m, press_at, temp_transl;
extern COORD_TYPE Ptens[3], DQtens[3], sqrtdr2, Aa, DrSqTot,
  DphiSq, PE[MAX_M*PE_POINTS], smed, PEijM[2*MAX_M*PE_POINTS];
extern int MB[NUMV], PEint[MAX_M][PE_POINTS];
#endif
