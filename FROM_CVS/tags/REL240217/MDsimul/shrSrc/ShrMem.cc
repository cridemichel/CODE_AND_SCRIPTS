...
if ((shmid[nbloc] = shmget(key,size,IPC_CREAT|IPC_EXCL|0644)) == -1)
  {
   printf("Shared memory segment exists - try as client\n");
...
void createsem(int nsems)
{
  ...
  if ( (semid = semget(key, nsems, IPC_EXCL | IPC_CREAT | 0644)) == -1 )
    {
      printf("Semaphore set already exists !!!!\n");
      printf("Remove using 'ipcs' to see the resource number\n");
      printf("and then typing: ipcrm sem <resource number>\n");
      exit(1);
    }
...
void removesem(void)
{
  ...
  semopts.val=0; 
  semctl(semid, 0, IPC_RMID, semopts); /* semopts->0 */
...
{
  semopts.val=0;
  return semctl(semid,numerosem,GETVAL,semopts); /* semopts -> 0 */
}
