...
void energy(void)
{
  ...
  if (OprogStatus.Nose > 0)
      ...
      if (OprogStatus.Nose == 2 || OprogStatus.Nose == 1)
	{
	  E = E + 0.5 * Sqr(s1) * OprogStatus.Q / Sqr(s) + 
	    (6.0 * ((COORD_TYPE) Nm) - 3.0) * Oparams.T * log(s);
	}
  ...
  Ly = 0.0;
  Lz = 0.0;
  Mtot = Oparams.m[0] + Oparams.m[1] + Oparams.m[2];
  cost = Vol1 / Vol / 3.0;
  L = cbrt(Vol);
  invL = 1.0 / L;
  ...
  for(i=0; i < Oparams.parnum; i++)
    {
      ...
      ryCM = (m0 * ry[0][i] +  m1 * ry[1][i] + m2 * ry[2][i]) * invMtot;
      rzCM = (m0 * rz[0][i] +  m1 * rz[1][i] + m2 * rz[2][i]) * invMtot;
