...
#define NUMK 99    /* number of k-points in which we must  calculate the 
		       structure factor */ 
#define MAXBIN 200  /* Number of radius in which calculate the radial 
...
struct pascii opro_ascii[] =
{
  ...
  {"kmax",         &OS(kmax),                       1,   1, "%.6G"},
  {"Dk",           &OS(Dk),                         1,   1, "%.6G"},
  {"inilattice",   &OS(inilattice),                 1,   1, "%d"},
...
struct singlePar OsinglePar[] = { 
  ...
  */
  ...
  {"inilattice", &OprogStatus.inilattice,     INT},
  ...
  {"DtrCalc",    &OprogStatus.measCalc[1],    INT}, /* steps between measure
						       calculation */
  {"DtrInit",    &OprogStatus.initStep[1],   INT},
  ...
  {"pressInit",  &OprogStatus.initStep[6],   INT},
  ...
  {"VcInit", &OprogStatus.initStep[11], INT},
  {"Q",          &OprogStatus.Q,              CT},
...
COORD_TYPE Ptens[3], DQtens[3], 
  sqrtdr2, Aa, DrSqTot, temp_transl;
int MB[NUMV];
#else 
extern COORD_TYPE E, Dtrans, temp, S[NUMK], dummy, eta, gr[MAXBIN], invs,
  press, press_m, press_at, temp_transli, Vc;
