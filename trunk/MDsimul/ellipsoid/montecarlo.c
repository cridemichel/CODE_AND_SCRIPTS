...
#if defined(MC_ELASTIC_CONSTANTS) || defined(MC_ELCONST_STRAIGHT)
  ...
  *merr=0;
  while (!bonded)
      ...
      else if (dist_type == 11 || dist_type == 12 || dist_type == 13)
	{
	  ...
	  //printf("i=%d u=%f %f %f\n", i, ox, oy, oz);
	  ec_segno[i]=1.0;
#elif defined(MC_ELCONST_STRAIGHT)
	  if (i == cur_ii || i == cur_jj)
	      ...
	      if (i < cur_jj)
		{
		  ...
		  oy = verso*ec_uy[0];
		  oz = verso*ec_uz[0];
		  //printf("i=0 %f %f %f i=size1 %f %f %f  i=%d %f %f %f cur_jj=%d\n", ec_ux[0], ec_uy[0], ec_uz[0], ec_ux[cur_jj], ec_uy[cur_jj], ec_uz[cur_jj], i, ox, oy, oz, cur_jj);
		}
	      else
		{
		  //printf("i=0 %f %f %f i=size1 %f %f %f  i=%d %f %f %f cur_jj=%d\n", ec_ux[0], ec_uy[0], ec_uz[0],
		  //	    ec_ux[cur_jj], ec_uy[cur_jj], ec_uz[cur_jj], i, ox, oy, oz, cur_jj);
		  ox = verso*ec_ux[cur_jj];
		  oy = verso*ec_uy[cur_jj];
		  oz = verso*ec_uz[cur_jj];
		}
	      ec_ux[i] = ox;
	      ec_uy[i] = oy;
	      ec_uz[i] = oz;
      ...
      if (trials > maxtrials)
	{
	  ...
	  printf("i nv=%f %f %f\n", nv[0], nv[1], nv[2]);
	  calc_nvec(dist_type, rx[j], ry[j], rz[j], qvecG, nv);
	  printf("j nv=%f %f %f\n", nv[0], nv[1], nv[2]);
	  printf("BOH %f %f %f\n", dx+norm*vv[0], dy+norm*vv[1], dz+norm*vv[2]);
#endif
	  *merr=1;
	  return;
	}
...
void calc_average_len_proj(int type, double alpha, long long int maxtrials, int outits, int size1)
{
  int k1, k2, i, j, kk, merr, selfoverlap=0, overlap, bt, nb, ierr;
  long long int tt;
  int ii, jj;
  double rA[3], rat[2][3],zMax, zMin;
  double delrxii, delryii, delrzii, delrxjj, delryjj, delrzjj;
  const int nn=1000;
  FILE *f;
  double cov=0.0, totene=0.0, shift[3], segno;
  double ox, oy, oz, Rl[3][3], Rcm1[3], Rcm2[3], Rcm[3], fact1, fact2; 
  
  f = fopen("avglen.dat","w+");
  fclose(f);
  ...
  while (tt < maxtrials) 
      ...
      if (tt%outits==0)// && ii==0 && jj==size1)
	{
	  if (tt!=0)
	    {
	      cov = (totene/((double)tt));
	      f = fopen("avglen.dat","a");
	      printf("avglen=%.10f (totene=%f/%lld)\n", cov, totene, tt);
      ...
      if (selfoverlap||merr)
	{
      ...
      if (selfoverlap == 0 && merr==0 && ierr==0)
	{
	  /* calc length of chain projection onto axis z */
	  zMax = zMin = 0.0;
	  for (i=0; i < size1; i++)
	    {
	      rA[0] = rx[i];
		...
		for (jj = 0; jj < 3; jj++)
		  RtA[ii][jj] = R[i][ii][jj];
	      BuildAtomPosAt(i,1, rA, RtA, rat[0]);
	      BuildAtomPosAt(i,2, rA, RtA, rat[1]);
	      if (rat[0][2] > zMax)
		zMax = rat[0][2];
	      else if (rat[0][2] < zMin)
		zMin = rat[0][2];
	      if (rat[1][2] > zMax)
		zMax = rat[1][2];
...
#ifdef ELCONST_NEWALGO
int create_pair_and_check(int type, int size1, double r1[3], double r2[3], double u1[3], double alpha, double qvec, double *totene, double *toteneExc, int use_stored_bonds)
{
#ifdef MD_LL_BONDS
  long long int aa, bb, ii, jj, jj2;
#else
  int ii, jj, aa, bb, jj2;
#endif
  int k1, k2, i, j, kk, merr, bt, nb, ierr, selfoverlap=0, overlap=0;
  long long int tt;
  double delrxii, delryii, delrzii, delrxjj, delryjj, delrzjj;
  const int nn=1000;
  FILE *f;
  double shift[3], segno;
  double ox, oy, oz, Rl[3][3], Rcm1[3], Rcm2[3], Rcm[3], fact1, fact2; 
      ...
      if (i==size1)
	{
	  ...
	  if (type==1)
	    ...
	    orient_onsager_field(rx[i], ry[i], rz[i], &ox, &oy, &oz, alpha, qvec, type);
     	  if (oz < 0.0)
    	    {
    	      ox=-ox;// since particle have head-tail symmetry we can change sign to orientation vector
    	      oy=-oy;
    	      oz=-oz;
    	    }
...
void calc_elastic_constants(int type, double alpha, double qvec, long long int maxtrials, int outits, int size1)
{
  ...
  long long int tt;
  int ii, jj;
  double r1[3], r2[3], u1[3], delrxii, delryii, delrzii, delrxjj, delryjj, delrzjj;
  const int nn=1000;
  FILE *f;
  double cov, covExc, totene, toteneExc, shift[3], segno;
  double ox, oy, oz, Rl[3][3], Rcm1[3], Rcm2[3], Rcm[3], fact1, fact2; 
  if (type == 11)
    f = fopen("v11.dat","w+");
  else if (type==12)
  ...
  while (tt < maxtrials) 
      ...
      if (tt%outits==0)// && ii==0 && jj==size1)
	{
	  if (tt!=0)
	    {
	      ...
	      covExc = (toteneExc/((double)tt))*(L[0]*L[1]*L[2]);
      ...
      r1[0]=r1[1]=r1[2]=0.0;
#ifdef MD_LXYZ
      r2[0] = L[0]*(ranf_vb()-0.5);
      r2[1] = L[1]*(ranf_vb()-0.5); 
      r2[2] = L[2]*(ranf_vb()-0.5); 
#else
      r2[0] = L*(ranf_vb()-0.5);
      r2[1] = L*(ranf_vb()-0.5);
      r2[2] = L*(ranf_vb()-0.5);
      ...
      if (u1[2] < 0.0)
	{
	  u1[0]=-u1[0];// since particle have head-tail symmetry we can change sign to orientation vector
	  u1[1]=-u1[1];
	  u1[2]=-u1[2];
	}
...
void calc_cov_additive(void)
{
 ...
 if (type==11||type==12||type==13)
  ...
  /* type = 0 -> covolume 
     ...
     type = 12 -> K22
     type = 13 -> K33
     type = 14 -> average chain length of projection onto z axis
   */
 if (type==1||type==5||type==7||type==8)
  ...
  if (type == 14)
    {
      calc_average_len_proj(type, alpha, maxtrials, outits, size1);
      exit(-1);
    }
  if (type == 11 || type == 12 || type == 13)
    {
