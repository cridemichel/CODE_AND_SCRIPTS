...
/* ============================= >>> BCC <<< ================================*/
void BCC(int Nm, COORD_TYPE m)
{
  int Nc;
  COORD_TYPE rRoot3; // = 0.5773503;
  COORD_TYPE  Cell, Cell2;
  int i, ix, iy, iz, iref, ii;
  COORD_TYPE bx[2], by[2], bz[2]; /* base vectors for FCC lattice */
  
  L = cbrt(Vol);
  Nc = ceil(  pow( ((COORD_TYPE)Nm)/2.0, 1.0/3.0 )  );
  ...
  loop(iz, 1, Nc) /* loops over unit cells (that are simply cubes) */ 
      ...
      loop(iy, 1, Nc)
	{
	  loop(ix, 1, Nc)
	    {
	      loop(iref, 1, 2) /* In each primitive cell there are four 
				  molecules */
		{
		  if ((ii + iref) >= Nm) break;
...
void initCoord(void)
{
  ...
  if (OprogStatus.inilattice==0)
    BCC(Oparams.parnum, Oparams.m); 
  else
...
void usrInitBef(void)
{
  ...
  OprogStatus.inilattice = 1;/*default 0=FCC*/ 
  Oparams.m = 1.0;
