...
void calcFsk(int kmod, int *Nm, int nf)
{
  ...
  /* iq: media su tutti i k che hanno modulo kmod e che si sono stati scelti come mesh */
  for(iq=1; iq < ntripl[kmod]; iq++)
    {
	  ...
	  for (i=0; i < Nm[0]; i++)
	    {
      	      rxdummy = (rrx[0][i]-rrx[i][jj])*mesh[kmod][iq][0]
...
int main(int argc, char** argv)
{
   ...
   * la S(k) è massima!!! */
  calcFsk(22, Oparams.parnum, njob);
  
  /* ============================================================================*/
  writeFsk(njob);
  mdPrintf(ALL, " Finito calcolo Fs(k)\n", NULL);
  return 0;
}
