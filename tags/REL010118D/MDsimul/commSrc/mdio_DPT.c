...
/* ============================ >>> openMPI <<< ============================ */
int openNewMPI_Tord(char* fn, int how, int perm)
{
  /* DESCRIPTION:
     Open an existant file.
     if MPI macro is defined open a file name adding _R<rank>, that is 
  ...
  strcpy(tmpStr, fn);
  /* 08/03/2001:
     questa funzione è usata da saveMeasure percio' non bisogna salvare
     in base al rank ma in base alla temperatura attuale!!! */
  //sprintf(fileName, "%s_R%d", tmpStr, my_rank);
  sprintf(fileName, "%s_T%d", tmpStr, Oparams.lambdat[my_rank]);
  //printf("leggo file: %s\n", fileName);
  fd = open(fileName, how, perm);
  return fd;
}
...
/* ============================ >>> openMPI <<< ============================ */
int openMPI_Tord(char* fn, int how)
{
  /* DESCRIPTION:
     if MPI macro is defined open a file name adding _R<rank>, that is 
     if <fn> is the file name passed by fn, then the file opened by this 
  ...
  int fd;
  strcpy(tmpStr, fn);
  sprintf(fileName, "%s_T%d", tmpStr, my_rank);
  fd = open(fileName, how);
  return fd;
}
...
/* ============================ >>> openMPI <<< ============================ */
int creatMPI_Tord(char* fn, int how)
{
  /* DESCRIPTION:
     if MPI macro is defined open a file name adding _R<rank>, that is 
     if <fn> is the file name passed by fn, then the file opened by this 
 ...
 */
  int fd;
  /* questa routine viene usata per creare i file di misura e gli xva file
     perciò i file devono avere estensione '_T<temperatura>' poiché
     i file di misura sono ordinati in base alla temperatura e non in base
     al rank */
  if (  ( fd = creatMPI_Tord(fileName,
		     0666) ) == -1  )
    {
...
/* =========================== >>> mdOpen <<< ===============================*/
int mdOpen_Tord( char* fileName, char* when, char* errMsg, int mode)
{
  /* DESCRIPTION:
     mode is the usual open mode except that you can also specify the EXIT
     flag or-ed with the usual flags ( actually EXIT = 0900000000 )*/
  ...
  if (  ( fd = openNewMPI_Tord(fileName, mode, 0666) ) 
       == -1  )
    {
      sprintf(msgStrA,"Can't open file %s for writing",
	      fileName);
...
void openLog(char* mode)
{
  ...
  sprintf(fn, "%s_R%d", absTmpHD(MDS_LOG), my_rank);
  stat(fn ,&OstatLog); 
...
void existDir(char* dirName)
{
  ...
  DIR* rfdir;
  rfdir = opendir(dirName);
  if (rfdir == NULL)
    {
      sprintf(msgStrA, "Not existing directory: %s", dirName);
