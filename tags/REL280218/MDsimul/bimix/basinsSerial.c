  ...
  for(iwork = 1; iwork <= njob; iwork++)
    {
	 ...
	 minimization configuration
      ...
      writeconf(filename[iwork]);
      writeEnergy(filename[iwork], Oparams.T, Oparams.curStep,
...
#else
	  rx[a][i] -= L*RINT_(invL*rx[a][i]);
  	  ry[a][i] -= L*RINT_(invL*ry[a][i]);
  	  rz[a][i] -= L*RINT_(invL*rz[a][i]);
#endif
	}
    }
}
...
#else
	  rx[a][i] = rx[a][i] - L * RINT_(invL * rx[a][i]);
	  ry[a][i] = ry[a][i] - L * RINT_(invL * ry[a][i]);
	  rz[a][i] = rz[a][i] - L * RINT_(invL * rz[a][i]);
...
void LinMin(int Nm[NA], double *Fret)
{
  int j, i, a;
  /*const nmax = 1000;*/
  const double tol=1.0E-10;//tol = 1.0E-10;
  ...
  if (FB_ >= FC_)
    {
      R = (BX_-AX_) * (FB_-FC_);
      Q = (BX_-CX_) * (FB_-FA_);
      U = BX_-((BX_-CX_)*Q - (BX_-AX_)*R)/(2.0*DSIGN(max(DABS(Q-R),TINY),Q-R));
      ULIM = BX_ + GLIMIT*(CX_-BX_);
      if((BX_-U)*(U-CX_) > 0)
	{
	  FU = func(U);
          if (FU < FC_)
