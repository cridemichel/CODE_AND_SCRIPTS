...
extern COORD_TYPE EE[MAX_M], Vc, V, W, K,
  WC, T1xx, T1yy, T1zz,
  T1xx, T1yy, T1zz, T1xy, 
...
extern int my_rank;
/* ========================== >>> ACForce <<< =========================== */
void ACForce(int Nm, COORD_TYPE kmax, COORD_TYPE Dk, C_T alpha, C_T S0, double Lambda)
{
  int ind, indMax, i;
	  ...
	  for(i = 0; i < Nm; i++)
	   {
	     ...
	     Fy[i] += Fyy;
	     Fz[i] += Fzz;
	     // Calcola il contributo al viriale
  ...
  /* mancava Lambda poiche' avevo si moltiplicato lal forza man non
     il potenziale PD */
  VAC *= 0.5*alpha*Lambda;
  //EE = VAC / Lambda;
  //VAC *= alpha;
}
	     ...
	     COORD_TYPE sigma, COORD_TYPE rcut, double Lambda)
{
  ...
  for (n = 0; n < nebrTabLen; n++)
	...
	{
	  //rab   = sqrt(rabSq);
#ifdef    ONLY_HARD_CORE
	  srij2 = sigmaSq / rijSq;
	  vij = srij2 * srij2 * srij2 * srij2 * srij2;	  
