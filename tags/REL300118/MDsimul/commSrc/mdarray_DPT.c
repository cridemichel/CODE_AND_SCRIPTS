...
extern void scaleVelocities(double fact);


/* =========================== >>> saveCoord <<< ===========================*/
void saveScaledCoord(char* fileName)
{
  /* DESCRIPTION:
     save last coordinates on file fileName */
  int cfd; /* descriptor of coordinate file named fileName */ 
  ...
  scaleVelocities(sqrt(l0[lt[my_rank]]));
  /* writes all arrays to the disk physically making a sync() */
  writeSegs(cfd, "End", "Error writing final coordinates", EXIT,
	    SEGSIZE, SAVE_LIST,
	    NULL);
  ...
  scaleVelocities(1.0/sqrt(l0[lt[my_rank]]));
...
{	
  ...
  if (!allocato)
    {
      /* in questo modo alloca le coordinate una sola volta e non tutte le 
	 colte che chiama la routine.
	 Talle routine infatti è chiamata piu' volte in whichCorrupted 
	 e chooseRestore */
 ...
 rerr |= -readSegs(bfd, "Restore", 
		   "Error reading extra coords from restore file", 
		   CONT, sizeof(COORD_TYPE), EXT_PT_SLST,
		   NULL);   /* NULL means: 'no more pointers to load' */
...
void saveCorAscii(void)
{
  ...
  sprintf(fn ,"CorT%.6G-%s.%d_R%d", 
	  Oparams.T/Oparams.lambda0[Oparams.lambdat[my_rank]], 
	  OprogStatus.nRun, Oparams.curStep, my_rank);
...
void saveBakAscii(char *fn)
{
  ...
  if (fn == NULL)
    {
      /* Cancella eventuali file di restart che hanno lo stesso nome
        di quello da salvare a parte il rank
       se esistono puo' voler dire due cose:
      1) un simulazione è ripartita ad un passo precedente rispetto al
      	 salvataggio di un file di restart
      2) si sta usando un directory già utilizzata */
      sprintf(cancella ,"rm %sCnfT%.6G-%s.%d_R*", OprogStatus.tmpPath, 
	      Oparams.T/Oparams.lambda0[Oparams.lambdat[my_rank]], 
	      OprogStatus.nRun, Oparams.curStep);
      if (OprogStatus.PT)
	   system(cancella);		
      sprintf(fileop ,"CnfT%.6G-%s.%d_R%d", 
	      Oparams.T/Oparams.lambda0[Oparams.lambdat[my_rank]], 
	      OprogStatus.nRun, Oparams.curStep, my_rank);
      if ( (bf = fopen(absTmpAsciiHD(fileop), "w")) == NULL)
	{
...
void saveBak(char *fileName)
{
  ...
  /* writes all arrays to the disk physically making a sync() */
  //printf("QUI!!!\n");
  writeSegs(bf, NULL, "Error writing coordinates array on restore file", 
	    EXIT, SEGSIZE, SAVE_LIST,
	    NULL);             /* NULL means: 'no more pointers to load' */  
...
void doubleBufStatus()
{
  ...
  /* Nel PT i processi devono essere sincroni dunque qui salva un solo
     file binario di restart per evitare casini
     PRIMA: appSw(absTmp(...),STA) ora STA -> 0*/
  
  sf = mdCreat( appSw(absTmpHD(STATUS_FILE_NAME),0), NULL,
		 ...
		 char* msgClose)
{
  ...
  if ( (mfd1 = mdOpen_Tord(fileName, NULL, msgOpen, CONT | O_WRONLY)) == -1)
    {
      return -1;
    }
...
void saveOneXva(char* fileName)
{
  ...
  xvafd = mdOpen_Tord(fileName, NULL, "unble to open xva file (tape file)", 
		 EXIT | O_WRONLY);
