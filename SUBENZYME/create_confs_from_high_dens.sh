...
if (at==1) {if (NR==ATA+1) print(nE, nS, "0"); else print $0;} else if (at==2) {if (NR-ATB<=nE) print $0; else if (NR-ATB<=nE+nS) print $0; \
if (NR-ATB > nT) { if (NR-ATB <= nT+nE) print $0; else if (NR-ATB <= nT+nE+nS) print $0; \
if (NR-ATB == 2*nT+1) print $0}}}' > start_NOCROWD_MS_${MS}_cnf
i=$[$i+1]
done
