...
int main(int argc, char **argv)
{
  ...
  permdiam=0.0001; /* 20 T * 0.63 nm dove 0.63 nm è la lunghezza per base stimata per ssDNA in BiophysJ 86, 2630 (2004) */  
  //permdiam=0.5;
  Len=4.0; /* 48 bp equal roughly to 16 nm */
  if (argc == 1)
   {

     printf("create_dimers_conf <conf_file_name> <nxmax> <nymax> <nzmax> <phi> <diam>\n"); 
     exit(-1);
   }
  f = fopen(argv[1], "w+");
  if (f==NULL)
